...
class Weather {
  ...
  get sunset() {
    return this.weather.query.results.channel.astronomy.sunset
  }
  ...
  showForecast() {
    ...
    this.forecast.forEach(day => {
      document.write('<tr>')
      document.write(`<td>${day.day}</td>
                      <td>${Math.fToC(day.high)}°C</td>
                      <td>${Math.fToC(day.low)}°C</td>
                      <td>${this.to_celsius(day.high)}°C</td>
                      <td>${this.to_celsius(day.low)}°C</td>
                      <td>${day.text}</td>`)
      document.write('</tr>')
    })
    document.write('<t/able>')
  }
}
